...
//info: do not allow activation if MMP 4.0+ is active
if ( is_admin() ) {
	include_once( ABSPATH . 'wp-admin' . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'plugin.php' );
	if (is_plugin_active('maps-marker-pro/maps-marker-pro.php') || class_exists('Maps_Marker_Pro')) {
		$version = get_option('mapsmarkerpro_version');
		$version = ($version) ? $version : '';
		die("[Leaflet Maps Marker 3.12.1 - activation failed]: Please deactivate Maps Marker Pro $version first.");
	}
	//info: die if pro version is active
	if (is_plugin_active('leaflet-maps-marker-pro/leaflet-maps-marker.php') ) {
		if (!is_multisite()) {
			exit('[Leaflet Maps Marker - activation failed]: Please deactivate "Maps Marker Pro" first before downgrading to the free version!<br/>Please tell us what we can do to win you as a happy pro user at <a href="https://www.mapsmarker.com/feedback" target="_blank">www.mapsmarker.com/feedback</a> and receive a discount voucher!');
		} else {
			if (is_network_admin()) {
...
class Leafletmapsmarker
{
	...
	function lmm_install_and_updates() {
		...
		$current_version = "v3122"; //2do - mandatory: change on each update to new version!
