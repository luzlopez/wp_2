...
if (version_compare(get_option('leafletmapsmarker_version'),'3.12.1','=')) {
	...
	if ( $version_before_update === FALSE ) {
		set_transient( 'leafletmapsmarker_version_before_update', 'MapsMarker-transient-for-dynamic-changelog', 60 );
		update_option('leafletmapsmarker_version_before_update', '3.12.1');
	}
	update_option('leafletmapsmarker_version', '3.12.2');
	//info: update MapQuest basemaps with OSM
	$mapquest_update_1 = "UPDATE `" . $table_name_markers . "` SET `basemap` = 'osm_mapnik' WHERE `basemap` = 'mapquest_osm';";
	$wpdb->query($mapquest_update_1);
	$mapquest_update_2 = "UPDATE `" . $table_name_layers . "` SET `basemap` = 'osm_mapnik' WHERE `basemap` = 'mapquest_osm';";
	$wpdb->query($mapquest_update_2);
	$mapquest_update_3 = "UPDATE `" . $table_name_markers . "` SET `basemap` = 'osm_mapnik' WHERE `basemap` = 'mapquest_aerial';";
	$wpdb->query($mapquest_update_3);
	$mapquest_update_4 = "UPDATE `" . $table_name_layers . "` SET `basemap` = 'osm_mapnik' WHERE `basemap` = 'mapquest_aerial';";
	$wpdb->query($mapquest_update_4);
	$mapquest_update_5 = "UPDATE `" . $table_name_markers . "` SET `basemap` = 'osm_mapnik' WHERE `basemap` = 'mapquest_hybrid';";
	$wpdb->query($mapquest_update_5);
	$mapquest_update_6 = "UPDATE `" . $table_name_layers . "` SET `basemap` = 'osm_mapnik' WHERE `basemap` = 'mapquest_hybrid';";
	$wpdb->query($mapquest_update_6);
	...
	set_transient( 'leafletmapsmarker_install_update_cache_v3122', 'execute install and update-routine only once a day', 60*60*24 );
}
